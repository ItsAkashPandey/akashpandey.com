...
export default function SkillsClient({ resolvedSkills }: { resolvedSkills: any[] }) {
  ...
  return (
    <article className="mt-8 flex flex-col gap-8 pb-16 relative">
      ...
      <div className="flex flex-col gap-8">
        {resolvedSkills.map((mainCat, idx) => (
          <div key={mainCat.id} className="space-y-6">
            ...
            <div className="flex flex-row-reverse items-center justify-between gap-4 sm:gap-12">
              <div className="shrink-0 scale-[0.6] sm:scale-100 origin-right transition-transform -mr-4 sm:mr-0 w-[120px] h-[200px] sm:w-[280px] sm:h-[250px]">
                <SwipeCards images={mainCat.images} className="w-full h-full" />
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch">
              {mainCat.subcategories.map((subcat: any, subidx: number) => (
                <div
                  key={subidx}
                ...
                >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 flex-1">
                    {subcat.tools.map((tool: any, toolIdx: number) => (
                      <div
                        key={toolIdx}
                        onClick={() => {
                        ...
                        className={`group flex items-center gap-3 px-3 py-2.5 rounded-lg bg-background/60 border border-border/20 hover:border-primary/40 transition-all duration-300 min-h-[54px] ${(tool as any).popupImages?.length > 0 ? "cursor-pointer" : ""}`}
                      >
